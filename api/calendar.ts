...
async function publishPost(req: VercelRequest, res: VercelResponse, session: Session) {
  ...
  const rows = await sql`
    SELECT sp.id, sp.platform, sp.caption, sp.media_url, w.id AS workspace_id
...
async function handler(req: VercelRequest, res: VercelResponse, session: Session) {
  ...
  if (req.method === 'POST') {
    const { day, hour, time, platform, caption, status, mediaUrl } = req.body || {};
    ...
    const rows = await sql`
      INSERT INTO scheduled_posts (workspace_id, day, hour, time_label, platform, caption, status, media_url)
      VALUES (${workspaceId}, ${day}, ${hour}, ${time}, ${platform}, ${caption}, ${postStatus}, ${mediaUrl || null})
