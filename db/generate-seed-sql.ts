...
const DEMO_ACCOUNT_NAME = 'TechyFuel (demo)';
const DEMO_EMAIL = 'demo@techyfuel.example';
// Password: DiPWkN8y7B7 — bcrypt hash below, generated once at authoring time.
const DEMO_PASSWORD_HASH = '$2a$10$maQTWiufdY3k0txz4V8CV.mhg9lzU6lYjqqUiphXlT2QFvfUuClc.';
...
out.push(`INSERT INTO accounts (name) VALUES (${esc(DEMO_ACCOUNT_NAME)});`);
out.push(`INSERT INTO users (account_id, email, password_hash)
  SELECT id, ${esc(DEMO_EMAIL)}, ${esc(DEMO_PASSWORD_HASH)} FROM accounts WHERE name = ${esc(DEMO_ACCOUNT_NAME)};`);
