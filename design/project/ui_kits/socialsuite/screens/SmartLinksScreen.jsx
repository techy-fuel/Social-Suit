function SmartLinksScreen() {
  const { Button, IconButton } = window.SocialSuiteDesignSystem_f55d92;
  const { smartLinkLinks } = window.SSMockData;
      ...
      <div style={{ flex: 1.4, display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <div style={{ background: 'var(--surface-card)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-xs)', padding: 18 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {smartLinkLinks.map((l, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
                <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <IconButton size="sm" icon={<i data-lucide="grip-vertical" style={{ width: 14, height: 14 }} />} label="Reorder" />
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ width: 260 }}>
        ...
        <div style={{ background: 'var(--slate-900)', borderRadius: 'var(--radius-xl)', padding: 12 }}>
          <div style={{ background: 'linear-gradient(180deg, var(--blue-royal), var(--blue-sky))', borderRadius: 'var(--radius-lg)', padding: '28px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }}>
            <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--accent-primary)' }}>HI</div>
            <div style={{ color: '#fff', fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 14 }}>HIRA Institute</div>
            <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 8 }}>
              {smartLinkLinks.map((l, i) => (
                <div key={i} style={{ background: 'rgba(255,255,255,0.92)', borderRadius: 'var(--radius-md)', padding: '10px 12px', fontSize: 11, color: 'var(--text)', textAlign: 'center', fontWeight: 500 }}>
                  {l.label}
                </div>
              ))}
            </div>
